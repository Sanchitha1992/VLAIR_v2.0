...
//                    alert("The visual property is going to create its own value");
            ...
            'newInConnection': function (options) {
              ...
              if ($.isArray(incommingValue)) {
                //incommingValue = incommingValue.value
                //incommingValue = incommingValue[0]
...
function showColorChooser(holderElement) {
  ...
  var colorChooserCanvas = $('<input />', {
    type: 'color', value: rgbToHex(parseInt(rTextField.val()), parseInt( gTextField.val()), parseInt(bTextField.val())), id: 'colorChooserCanvas', style: 'margin-top: -5px; width: ' + colorCanvasWidth + 'px; height: ' + colorCanvasHeight + 'px; background-color: #fff; border-color: #000; border-style: solid; border-width: 1px;'
});

    colorChooserCanvas.change(function(e){colorChange(e)});
    ...
    function textBoxChanged() {
        var red = parseInt($('#rTextField').val());
        var green =parseInt($('#gTextField').val());
        var blue =parseInt($('#bTextField').val());
        selectedColor = 'rgb(' + red + ',' + green + ',' + blue + ')';
        debugger;
        $('#colorChooserCanvas').val(rgbToHex(red, green, blue));
    }
    ...
    rTextField.keyup(textBoxChanged);
    gTextField.keyup(textBoxChanged);
  bTextField.keyup(textBoxChanged);
