...
function onDataFileReadComplete(event, file) {
    ...
    if (isJSONFile.exec(file.name)) {
        ...
        var canvasActualCenter = getActualCanvasBottom();
        aDataWidget.left = 70;
        aDataWidget.top = 20;
        aDataWidget.setCoords();
...
function hammerEventOverCanvas(ev) {
    console.log('hammerEventOverCanvas')
...
function getActualCanvasBottom() {
...
function canvasDropFunction(ev, ui) {
  debugger;
    //if(recordedEvents==null){
     //   recordedEvents=[{event:ev,ui:ui}];
    //}else{recordedEvents.push({event:ev,ui:ui});}
